...
```{r}
train %>% filter(edyrs==0) %>% kable_format()
```
...
# Dummies and others
train <- train %>% mutate(total_exp = expf + expp,
...
# Boxplot
train %>% filter(!(occupation=="artist" & lnwage>4)) %>% ggplot(aes(x = reorder(occupation, lnwage, FUN = median), y = lnwage, fill = as_factor(occupation))) +
  stat_boxplot(geom = "errorbar") +
  geom_boxplot() +
  theme(legend.position = "hide", axis.text.x = element_text(size=8, angle=45)) 

# Checking mean edyrs by occupation
train %>% 
  group_by(occupation) %>%
  dplyr::summarise(N = n(),
                   mean = mean(edyrs),
...
# Boxplot
train %>% ggplot(aes(x = reorder(occupation, edyrs, FUN = median), y = edyrs, fill = as_factor(occupation))) +
  stat_boxplot(geom = "errorbar") +
  geom_boxplot() +
  theme(legend.position = "hide", axis.text.x = element_text(size=8, angle=45)) 
...
We can see that:
* There's a large variance in `manager`, `sales`, `officeadmin`, `production`, `transport`

## Some trials...
```{r}
train %>% 
  ggplot(aes(x = region, y = lnwage, fill = as_factor(female))) +
  stat_boxplot(geom = "errorbar") +
  geom_boxplot() 

train %>% 
  ggplot(aes(x = race, y = lnwage, fill = as_factor(female))) +
  stat_boxplot(geom = "errorbar") +
  geom_boxplot() 

train %>% 
  ggplot(aes(x = educ, y = lnwage, fill = as_factor(female))) +
  stat_boxplot(geom = "errorbar") +
  geom_boxplot() 
